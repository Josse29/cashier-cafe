...
const ResetPassword = () => {
  ...
  return (
    <View>
      {loading && <Spinner />}
      {!loading && (
        <View className="border border-[#964a3b] rounded-lg w-full p-4">
          {/* alert */}
          <Alerts status="error" msg={errMsg} setMsg={setErrMsg} />
          {/* password */}
          <InputPassword color="#964a3b" setReq={setReq} value={req.password} />
          ...
          {/* btn submit */}
          <TouchableOpacity
            className="bg-[#964a3b] p-3 rounded-md mb-4"
